...
//默认含中间件的路由设定
type HandlerMidel func(http.ResponseWriter, *http.Request)

// ServeHTTP calls f(w, r).
func (f HandlerMidel) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if (!MidleCheck(w,r)){
		return
	}
	f(w, r)
...
func (mux *ServeMux) HandleFuncMiddle(pattern string, handler func(http.ResponseWriter, *http.Request)) {
	mux.Handle(pattern, HandlerMidel(handler))
}
// HandleFunc registers the handler function for the given pattern
// in the DefaultServeMux.
// The documentation for ServeMux explains how patterns are matched.
func HandleFuncMiddle(pattern string, handler func(http.ResponseWriter, *http.Request)) {
	DefaultServeMux.HandleFuncMiddle(pattern, handler)
}
